"""提供应用各层使用的日志配置, 记录和格式化能力

日志和提示文本是两件事, 刻意不合并:

- 提示文本 (`core.messages`) 面向用户: 措辞稳定, 将来要翻译, 是契约的一部分
- 日志面向开发者: 细节详细, 措辞随时会改, 不属于契约

因此本模块不存放任何面向用户的文案, 各层也不要把日志消息当成错误提示来用.

分工约定:

- 只由进程入口 (`main.py`, `cli.py`) 调用 `configure_logging`, 且只调用一次
- 库代码只用 ``logging.getLogger(__name__)`` 取日志器, 不自行添加处理器,
  不设置级别, 不修改全局配置
- 不写"进入函数 / 离开函数"这类流水账, 只记录业务上有意义的事件
- 异常只在出口层记录一次. 中间层要么处理掉, 要么原样抛出, 不要既记录又抛出,
  否则同一个故障会在日志里重复出现
- 需要附带变量时用 ``extra`` 传结构化字段, 不要拼进消息字符串::

      logger.info("导入完成", extra={"batch_id": batch.id, "count": count})

  这种写法既能被文本格式渲染成 ``batch_id=... count=...``, 也能被 JSON 格式
  原样输出交给日志系统检索
- 不记录密码, 密码散列, 令牌等敏感值. 本模块会对疑似敏感的字段名做兜底替换,
  但兜底只用于防手滑, 不能替代"不要传进来"这条纪律
"""

...
def import_backup(
    ...
    已知安全面: 本接口接收任意服务器本地路径, 因此调用方可以让服务端读取
    任意文件. 解析失败会报错, 但文件摘要会被写入 import_batches, 等于可以
    确认 "某个文件与我手上的文件是否相同". 当前部署形态下这是有意设计:
    程序只在本地运行, 调用方就是部署者本人, 读自己的文件不是越权; 且
    python-multipart 尚未安装, UploadFile 不可用, 走路径是唯一可行方案.
    接入真正的认证时, 本接口必须一并处理: 改为接收上传内容, 或把路径限制在
    配置好的导入目录内 (用 Path.resolve() 后检查前缀, 并注意符号链接).
    现在改成上传反而是错的: 在没有认证的前提下, 那只是把 "读任意文件"
    换成 "写任意文件", 没有解决根本问题.
    """
